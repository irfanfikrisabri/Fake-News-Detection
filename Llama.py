...
def post_process_output(output):
    ...
    if "real news" in output:
        ...
        # Default to "Real News" if no valid category is found
        return "Real News"

# Map predicted categories to labels
...
# Create pipeline with chat template support
    ...
    for i in range(0, len(texts), batch_size):
        ...
        try:
            for text in tqdm(batch_texts):
                ...
                # Print for debugging (only for a few samples)
                if i < 2:
            ...
            print("Trying to continue with reduced batch size...")
            ...
            if batch_size > 1:
                ...
                print("Unable to process even with batch size of 1, skipping this batch")
                all_predictions.extend(["Real News"] * len(batch_texts))  # Default prediction
    
    return all_predictions

# Make predictions in batches
print("Making predictions with Llama model...")
...
test_chunk.to_csv(f'/content/drive/My Drive/TYP/Llama/llama_optimized_predictions_{start_index}_{end_index}.csv', index=False)
