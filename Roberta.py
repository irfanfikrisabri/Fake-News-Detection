...
# Load RoBERTa tokenizer (CHANGED TO RoBERTa)
...
# Load pre-trained RoBERTa model (CHANGED TO RoBERTa)
...
def train(model, train_loader, val_loader, optimizer, scheduler, epochs, eval_steps=50):
    model.train()
    best_val_accuracy = 0
    best_model_path = '/content/drive/My Drive/TYP/RoBERTa/best_roberta_model.pth'
    output_lines = []
...
def evaluate(model, dataloader):
    ...
    predictions_df.to_csv('/content/drive/My Drive/TYP/RoBERTa/roberta_predictions.csv', index=False)
...
output_file_path = '/content/drive/My Drive/TYP/RoBERTa/roberta_training_output.txt'
with open(output_file_path, 'w') as f:
    for line in output_lines:
        f.write(line + "\n")

print(f"\nOutput saved to {output_file_path}")
print("Per-sample predictions saved to /content/drive/My Drive/TYP/RoBERTa/roberta_predictions.csv")